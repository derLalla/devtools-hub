...
async function loginAndGetToken(): Promise<string> {
  const res = await request(app).post('/api/auth/login').send({ username: 'admin', password: 'changeme' });
...
describe('links', () => {
  ...
  it('full CRUD lifecycle with admin token', async () => {
    ...
    const created = await request(app)
      ...
      .send({ title: 'Grafana', url: 'https://grafana.example.com', icon: 'lucide:bar-chart', sortOrder: 1 });
